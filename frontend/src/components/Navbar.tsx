...
import { Menu, X, ShoppingCart, User, LogOut, CreditCard, LayoutDashboard, ChevronDown, Crown } from "lucide-react";
...
export function Navbar() {
  ...
  const tierLabel = membershipTier !== 'free' ? membershipTier.charAt(0).toUpperCase() + membershipTier.slice(1) : '';
    ...
    <header className="sticky top-0 z-50 border-b bg-white/95 backdrop-blur shadow-sm">
      ...
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 lg:px-8">
        ...
        <nav className="hidden items-center gap-6 lg:flex">
          ...
          <Link
            href="/premium"
            className="flex items-center gap-1.5 text-sm font-bold text-amber-600 hover:text-amber-500 transition relative group"
          >
          ...
          {isAuthenticated ? (
            <div className="relative">
              ...
              >
                ...
                {isPremium && (
                  <span className="hidden md:inline text-[10px] bg-gradient-to-r from-amber-400 to-yellow-500 text-slate-900 font-bold px-1.5 py-0.5 rounded-full">
                    👑 {tierLabel}
                  </span>
                )}
                ...
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white border border-gray-100 rounded-xl shadow-xl z-50 py-2">
                    <div className="px-4 py-2 border-b border-gray-100">
                      ...
                      {isPremium && (
                        <span className="inline-flex items-center gap-1 text-[10px] bg-gradient-to-r from-amber-400 to-yellow-500 text-slate-900 font-bold px-2 py-0.5 rounded-full mt-1">
                          <Crown className="w-3 h-3" /> {tierLabel} Member
                        </span>
                      )}
                    </div>
                    ...
                    <Link href="/premium/dashboard" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm font-semibold text-amber-600 hover:bg-amber-50">
                      <Crown className="w-4 h-4 text-amber-500" /> {isPremium ? 'Premium Dashboard' : 'Upgrade to Premium'}
                    </Link>
                    {user?.role === 'supplier' && (
        ...
        <div className="border-t bg-white px-6 py-4 lg:hidden">
          <nav className="flex flex-col gap-1">
            ...
            <Link
              href="/premium"
              onClick={() => setMobileOpen(false)}
              className="flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-bold text-amber-600 hover:bg-amber-50"
            >
