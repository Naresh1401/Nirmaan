        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {filtered.map(worker => (
            <div key={worker.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all flex flex-col overflow-hidden">
              ...
              {isAuthenticated ? (
                <button onClick={() => showToast(`Hire request sent to ${worker.name}! 📩`)} className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 text-sm transition-all flex items-center justify-center gap-2">
